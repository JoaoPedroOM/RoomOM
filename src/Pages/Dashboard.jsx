...
const Logo = React.memo(() => (
  <div className="text-left py-4 flex items-center">
    <h2 className="text-xl font-bold text-white">RoomOM</h2>
  </div>
));
...
const Dashboard = () => {
  ...
  return (
    <div className="lg:flex lg:flex-row flex flex-col h-screen">
      <Sidebar open={open} setOpen={setOpen}>
        <SidebarBody className="justify-between lg:gap-10 gap-2">
          <div className="flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
            {open ? <Logo /> : <LogoIcon />}
            <div className="lg:mt-8 mt-2 flex flex-col gap-2 font-semibold">
              {links.map((link, idx) => (
      ...
      <main className="flex-1 p-4 text-white">Main e seus dados aqui</main>
    </div>
  );
};
